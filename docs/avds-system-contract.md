...
компоненты, shell и зарегистрированные композиции. Текущий `N` — `98%`
(`126/128`): screen-reader и zoom acceptance, периодное сравнение и visual
regression закрыты отдельными доказательствами. Не засчитываются только полные
KK/EN пользовательские интерфейсы.
...
а также нормы чисел, дат, единиц, переносов и сообщений empty/error ещё не
закрыты. Их отсутствие снижает общую метрику и должно оставаться видимым.
